...
import { getStrictness } from './settings.service.js';
import { PROMPTS } from '../config/ai_prompts.js';
...
export async function parseTextWithOpenAI(text, hints = "") {
  ...
  const strictnessLevel = getStrictness();
  const strictnessPrompt = PROMPTS[strictnessLevel] || PROMPTS[5];
